...
class SupervisorView extends Component {
  ...
  render() {
    ...
    let results = metrics.map((result, index) => {
      let conversion =
        result.calls === 0 ? 100 : (result.sales / result.calls) * 100;
      ...
      return (
        <tr key={index}>
          ...
          <td>{result.sales}</td>
          <td>{result.calls}</td>
          <td className={`alert-${this.statusClass(conversion)}`}>
      ...
      <Grid>
        ...
        <Row>
          ...
          <Col md={4}>
            <h2 className="center">
              Conversion{' '}
              <Badge className={`alert-${this.statusClass(conversion)}`}>
